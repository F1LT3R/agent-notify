  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description: "Lightweight status check for turn-taking and polling. Returns only counters (latest_id, played_id, has_new, queue_length) — no message bodies. Use this instead of get_messages when you only need to check if audio has finished playing or if new messages exist. Saves tokens by returning ~30 tokens instead of ~400-600.",
        inputSchema: {
          type: "object",
          properties: {
            since_id: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "notify") {
    const { type, message, workspaceDir, agentRole, agentNumber, voice, model, to } = request.params.arguments;
