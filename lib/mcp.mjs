        ...
        inputSchema: {
          ...
          properties: {
            type: {
              ...
              enum: ["question", "permission", "done", "error", "status", "waiting", "review", "message", "image"]
            },
            message: {
              type: "string",
            ...
            image: {
              type: "string",
              description: "Absolute path to an image (PNG/JPEG/GIF/BMP/TIFF/SVG) to render inline in the iTerm2 terminal. Can accompany any notification type (e.g., 'done' with a screenshot, or type 'image'); the image is printed after the message text."
            }
          },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "notify") {
    const { type, message, workspaceDir, agentRole, agentNumber, voice, model, to, response_to, image } = request.params.arguments;
    ...
    if (image !== undefined) params.set('image', image);
