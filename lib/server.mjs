...
const MESSAGE_STORE_PATH = STORE_PATH
...
const META_PATH = MESSAGE_STORE_PATH + '.meta';
let lastWrittenId = 0;
...
function loadMessageStore() {
  if (CLEAR) {
    for (const f of [MESSAGE_STORE_PATH, META_PATH]) {
  ...
  const OLD_STORE = path.join(
    __dirname, '..', '.message-store.json'
  );
  if (!fs.existsSync(MESSAGE_STORE_PATH)
      && fs.existsSync(OLD_STORE)) {
    const old = JSON.parse(
      fs.readFileSync(OLD_STORE, 'utf-8')
    );
  ...
  if (messageStore.length > 0) {
    const ts = new Date().toISOString()
      .replace(/[:.]/g, '-');
    const safetyPath = MESSAGE_STORE_PATH
      + `.pre-start.${ts}`;
    fs.copyFileSync(MESSAGE_STORE_PATH, safetyPath);
    console.log(
      `🛡️  Startup backup: ${safetyPath}`
    );
  }
...
app.get('/messages', (req, res) => {
  ...
  res.json({ messages: results, latest_id: messageIdCounter, last_played_id: playedId, muted: muteMessages });
});
...
// Render last 4 real messages on startup
if (messageStore.length > 0) {
...
async function startupConfirm() {
  ...
  // Temporarily disable raw mode for readline prompt
  if (process.stdin.isTTY) {
  ...
  return new Promise((resolve) => {
    rl.question('   Continue? [y/N] ', (answer) => {
      ...
      // Re-enable raw mode for keyboard controls
      if (process.stdin.isTTY) {
        process.stdin.setRawMode(true);
...
if (WATCH_MODE) {
  ...
  const server = app.listen(PORT, HOST, () => {
    startupConfirm().then(() => {
      // Port confirmed — safe to write to store
