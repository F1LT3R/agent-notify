...
function loadMessageStore() {
  if (CLEAR) {
    const hasStore = [MESSAGE_STORE_PATH, META_PATH]
      .some(f => fs.existsSync(f));
    if (!hasStore) {
      console.log('🗑️  Nothing to clear');
      return;
    }
    // Defer actual deletion to after confirm
    return;
  }
...
function setupPrimaryKeyboard() {
  ...
  process.stdin.on('keypress', (str, key) => {
    ...
    if (key && key.name === 'm') {
      ...
      const muteMsg = muteMessages
        ? '🔇 Agent messages muted'
...
// Load persisted messages (server mode only)
if (!WATCH_MODE) {
  loadMessageStore();

  if (CLEAR) {
    clearStoreConfirm().then((confirmed) => {
      if (!confirmed) {
        console.log('Aborted.');
        process.exit(0);
      }
      if (fs.existsSync(MESSAGE_STORE_PATH)) {
        ...
        fs.copyFileSync(
          MESSAGE_STORE_PATH, backup
        );
        console.log(
          `🛡️  Backup: `
          + osc8(backup, path.basename(backup))
        );
      }
      for (const f of [
