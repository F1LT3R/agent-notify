...
import fs from 'fs';
import displayImage from 'iterm2-image';
import crypto from 'crypto';
import { fileURLToPath } from 'url';
import {
  initPush, getVapidPublicKey,
...
// Print an image inline in the iTerm2 terminal via the iterm2-image package,
// followed by a clickable link to the image file under the image.
function printIterm2Image(absPath) {
  return new Promise((resolve) => {
    try {
      displayImage(absPath, (err) => {
        if (err) {
          console.log(`\x1b[2m🖼️ (image not rendered: ${absPath} — ${err.message})${resetColor}`);
        } else {
          console.log(`\x1b[2m🔗 ${osc8(absPath, absPath)}${resetColor}`);
        }
        resolve();
      });
    } catch (err) {
...
function storeMessage(entry) {
  ...
  // Web Push — skip internal and noisy low-level messages
  if (
    stored.type !== 'played' &&
    stored.type !== 'trace' &&
    stored.type !== 'debug' &&
    ...
    const emoji = agentEmojiMap[stored.type]
      || appEmojiMap[stored.type] || '';
    sendPushToAll({
      title: stored.project
        ? `${emoji} ${stored.project}`
        : `${emoji} Agent-Notify`,
      body: `${(stored.type || '').toUpperCase()} — `
        + stored.message,
      icon: '/logo.png',
      tag: `an-msg-${stored.id}`,
      data: {
        id: stored.id,
        type: stored.type,
        url: '/'
...
app.get('/notify/agent', async (req, res) => {
  const { type, message, workspaceDir, agentRole, agentNumber, voice, model, to, image } = req.query;
  ...
  if (!soundFile) {
    return res.status(400).json({
      error: `Unknown notification type: ${type}. Valid types: question, permission, done, error, status, waiting, review, message, image`
    });
  }

  // Image validation — any type may carry an image
  if (image) {
    let imgStat = null;
    try { imgStat = fs.statSync(image); } catch { /* missing */ }
    if (!image.startsWith('/') || !imgStat || !imgStat.isFile()) {
      return res.status(400).json({
        error: `Image must be an absolute path to an existing file: ${image}`
      });
    }
  } else if (type === 'image') {
    return res.status(400).json({
      error: "Type 'image' requires an 'image' parameter (absolute file path)"
    });
  }
  ...
  console.log(`"${message}"\n`);
  if (image) {
    await printIterm2Image(image);
    console.log();
  }
  printStickyLink();

  // Store message
  const stored = storeMessage({ source: 'agent', header, type, message, project, agentRole, agentNumber: agentNum, model, voice: selectedVoice, to, responseTo, image: image || undefined });
...
app.get('/notify/app', async (req, res) => {
  ...
  // Build console log — always shown
  ...
  // Always store (web UI sees all levels)
  const stored = storeMessage({ source: 'app', header, type, message, project, detail, app: appName, url });

  // Audio gated by --log-level and --log-level-audio
  if (!meetsThreshold(type, LOG_LEVEL) || !meetsThreshold(type, LOG_LEVEL_AUDIO)) {
    markPlayed(stored.id);
    return res.json({
      success: true,
      queued: false,
    ...
    if (recent.length > 0) {
      ...
      for (const msg of recent) {
        ...
        if (msg.image && fs.existsSync(msg.image)) {
          await printIterm2Image(msg.image);
          console.log();
        }
        console.log();
      }
...
if (WATCH_MODE) {
  ...
  server.on('listening', async () => {
    ...
    renderRecentMessages().then(() => printStickyLink());
  });
  server.on('error', (err) => {
