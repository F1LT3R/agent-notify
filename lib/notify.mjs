...
if (args.length < 2) {
  console.error('Usage: notify <type> <message> [--workspace-dir path] [--agent-role name] [--agent-number N] [--voice voice] [--model model] [--app name]');
  process.exit(1);
}
...
for (const [key, value] of Object.entries(optionalParams)) {
  // Skip workspaceDir for app notifications
  if (endpoint === '/notify/app' && key === 'workspaceDir') {
    continue;
  }
  params.set(key, value);
}
const curlCommand = `curl -s "http://192.168.0.6:8881${endpoint}?${params.toString()}"`;
