...
| `response_to` | No       | Message ID this is a reply to. Enables lightweight response polling via `check_responses_available` and `check_responses_observed`. |
...
```json
{
  "latest_id": 47, "last_played_id": 45, "muted": false, "has_new": true, "queue_length": 2,
  "agents": [
...
## Response Polling (Lightweight Turn-Taking)

For agent-to-agent conversations, use response polling instead of `check_message_status` to save tokens (~5 tokens per poll vs ~50-100).

### How it works

1. When replying to a message, pass `response_to=<id>` in your `notify` call
2. The other agent polls for your response using one of two modes

### Bus mode: `user-agent-notify-check_responses_available`

"Has a reply been sent?" — doesn't care if the human heard it. Use for task handoffs, status updates, anything where audio doesn't matter.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `id`      | Yes      | Message ID to check for responses to |

**Response:** `{"n":1}` or `{"n":0}`

### Conversational mode: `user-agent-notify-check_responses_observed`

"Has the reply been heard?" — only counts responses whose audio has been played. Use for debates, turn-taking, anything where timing matters.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `id`      | Yes      | Message ID to check for observed responses to |

**Response:** `{"n":1}` or `{"n":0}`

### Flow

1. **Send** with `response_to` pointing at the message you're replying to:
   ```
   user-agent-notify-notify type="message" message="..." response_to=225 → id: 226
   ```

2. **Other agent polls** (every 30s):
   ```
   user-agent-notify-check_responses_observed id=226 → {"n":0}  # not heard yet
   user-agent-notify-check_responses_observed id=226 → {"n":1}  # heard — proceed
   ```

3. **Read reply content** (if needed):
   ```
   user-agent-notify-get_messages response_to=226
   ```
...
2. **Wait** for audio to finish — poll `check_message_status` until `last_played_id >= 47`:
   ```
   user-agent-notify-check_message_status since_id=46 → { last_played_id: 46, has_new: true }  # still playing
   user-agent-notify-check_message_status since_id=46 → { last_played_id: 47, has_new: true }  # done — send next turn
   ```
...
- The user can press `[M]` to mute all audio while still seeing messages in the console
